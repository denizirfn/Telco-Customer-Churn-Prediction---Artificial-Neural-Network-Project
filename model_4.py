...
model3 = KerasClassifier(model=build_model, input_dim=input_dim, learning_rate=0.001, dropout_rate=0.5, epochs=50, batch_size=16, verbose=0)

# Sınıf ağırlıklarını ayarlayalım
model3.set_params(class_weight=dict(enumerate(class_weights)))

# Cross-validation işlemi
cv_results = cross_validate(model3, X, y, cv=10, scoring=["accuracy", "f1", "roc_auc"])
